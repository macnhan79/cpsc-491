...
public partial class UsersManagement : System.Web.UI.Page
{
    ...
    {
        string sql = ASPxMemo1.Text.Trim();
        SqlHelperWeb helper = new SqlHelperWeb(DA_Connection.GetConnectionString());
        if (sql.StartsWith("select"))
        {
            gridDictionary.DataSource = helper.ExecuteDataTable(sql, CommandType.Text, null, null);
            gridDictionary.DataBind();
        }
        else
        {
            ASPxLabel1.Text = helper.ExecuteNonQuery(sql, CommandType.Text, null, null) + string.Empty;
        }
